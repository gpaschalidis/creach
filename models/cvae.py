...
class CReach(nn.Module):
    ...
    def encode(self, enc_x,epoch_num,it):
        #if epoch_num ==2 and it ==1369:
        #    import pdb
        #    pdb.set_trace()
        ...
        #if epoch_num == 3 and it ==469:
        #    var.retain_grad()
        #var.retain_grad()
        try:
            george = torch.distributions.normal.Normal(mean, var)
        except:
            print("Value-error")
            import pdb
            pdb.set_trace()
        return george, mean, var_linear, var, X

    def decode(self, dec_x, wrist_transl):
        ...
        #trans = self.dec_trans(X) + wrist_transl
        trans = self.dec_trans(X)
        #pose = self.dec_pose(torch.cat([dec_x[:,0][...,None], X], dim=1))
        #pose = self.dec_pose(torch.cat([trans[:,1][...,None], X], dim=1))
        #pose = self.dec_pose(torch.cat([trans, X], dim=1))
        pose = self.dec_pose(X)
        #pose = self.dec_pose(trans)
        ...
        #return {'pose':pose, 'trans':trans,'gaze':gaze,'dir':dir_}
        #return {'pose':pose, 'trans':trans,'dir':dir_}
        #return {'pose':pose, 'trans':trans,'gaze':gaze}
